### HXIME v0.9.5 VRChatワールド用 多言語入力キーボード
...
`Dicts` にはそのまま取り込める Google Mozc の日本語辞書と、韓国国立国語院の韓国語基礎辞書を用意しています（コンパクト版と完全変換版）。ファイルの選び方、出典、ライセンスは[辞書について](Dicts/SOURCES.md)を参照してください。

現在の `HXIME_Pinyin.prefab` は日本語・韓国語のコンパクト辞書を既定で接続済みなので、すぐに言語を切り替えられます。古いシーンでは `Tools → HXIME → Configure Japanese and Korean Dictionaries` を実行してください。詳しい設定手順、辞書のファイル形式、機能範囲は[多言語辞書入力](MULTILINGUAL-JP.md)にまとめています（[中文](MULTILINGUAL.md) ｜ [English](MULTILINGUAL-EN.md) ｜ [한국어](MULTILINGUAL-KO.md)）。

中国語・日本語・韓国語は同一のソート済み索引と Top-30 の候補検索を共有し、中国語は逆方向前方一致と簡拼も保持します。索引は辞書の取り込み時、Play モード進入時、ワールドビルド時にエディターで生成されるため、キー入力時は関連する範囲だけを検索します。古いシーンや手動で変更した辞書は `Tools → HXIME → Rebuild All Dictionary Indexes` で再構築し、シーンを保存してください。カスタムスクリプトが実行時に重みを変更する場合はエンジンの `InvalidateMatchCache()` を呼び出します。コードや語句の構成はエディターで変更し、その後に索引を再構築してください。経緯・索引構造・検証範囲は[候補検索の最適化](PERFORMANCE-JP.md)にまとめています（[中文](PERFORMANCE.md) ｜ [English](PERFORMANCE-EN.md) ｜ [한국어](PERFORMANCE-KO.md)）。
...
- 多言語入力を使う場合は[多言語辞書入力](MULTILINGUAL-JP.md)に従って辞書を設定します。現在のプレハブは設定済みです。
- 完了です！
...
- 「ファイルを参照」と「辞書を読み込んで適用」で UTF-8 テキスト辞書を取り込みます。形式は「語句 tab コード tab 重み（省略可、既定 0）」です。拡張言語のコードはローマ字で、大文字小文字は区別しません。
...
- 出典・変換規則・ライセンス全文: [Dicts/SOURCES.md](Dicts/SOURCES.md)
