...
The current `HXIME_Pinyin.prefab` is wired to a 30,000-entry compact Japanese dictionary and a 10,000-entry compact Korean dictionary, so using that prefab cycles Chinese → Ja → Ko → En out of the box.
...
5. Compile UdonSharp, verify in Unity Play Mode / VRChat ClientSim, then build the world. The language button cycles Chinese → Ja → Ko → English → Chinese, skipping empty references.
...
`HXIMEUI.Target Inputfield` must point to a dedicated output field, never to `InputBarHandle/InputBar/InputField` (the pinyin composition field). A wrong binding makes committed text trigger a pinyin candidate refresh; the current version refuses to commit through such a binding. The menu **Tools → HXIME → Repair Output Field and Verify Chinese Selection** repairs this in the current scene: it backs up the scene into `Temp`, creates a dedicated `OutputField` under the candidate bar, rebinds it and saves the scene. An already correct output field is left untouched.
...
- Verify font rendering and large-dictionary input latency on the target platform. Extra dictionaries are scanned entry by entry, and the candidate buffer is tied to the candidate limit; large dictionaries still need real performance testing.
